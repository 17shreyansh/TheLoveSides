...
export default function NavbarRibbon({ isVisible }) {
  ...
  return (
    <div 
      className={clsx(
        "w-full bg-[#fdecf4] text-charcoal flex items-center justify-center relative transition-all duration-300 ease-in-out",
    ...
    >
      <button 
        onClick={handlePrev} 
        className="absolute left-2 md:left-4 p-1 hover:text-pink-primary transition-colors focus:outline-none z-10"
      ...
      <div className="flex-grow flex justify-center items-center h-full max-w-[85%] mx-auto py-2">
        <AnimatePresence mode="wait">
          ...
          >
            <Gift className="w-3 h-3 md:w-4 md:h-4 text-pink-primary hidden sm:block flex-shrink-0" />
      ...
      <button 
        onClick={handleNext} 
        className="absolute right-2 md:right-4 p-1 hover:text-pink-primary transition-colors focus:outline-none z-10"
