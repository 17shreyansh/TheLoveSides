...
export default function SubCategoryForm() {
  ...
  const fetchInitialData = async () => {
    try {
      const catsRes = await api.get('/admin/catalog/categories');
      setCategories(catsRes.data.data || []);
      
      if (isEditing) {
        const { data } = await api.get(`/admin/catalog/subcategories/${id}`);
        setFormData({
          ...data.data,
          categoryIds: data.data.categoryIds?.map(c => typeof c === 'string' ? c : c._id) || []
        });
      }
    } catch (err) {
  ...
  const handleCategoryToggle = (categoryId) => {
    setFormData(prev => {
      ...
      if (ids.includes(categoryId)) {
        return { ...prev, categoryIds: ids.filter(id => id !== categoryId) };
      } else {
        return { ...prev, categoryIds: [...ids, categoryId] };
      }
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await api.patch(`/admin/catalog/subcategories/${id}`, formData);
      } else {
        await api.post('/admin/catalog/subcategories', formData);
      }
  ...
  if (loading) return <div className="animate-pulse p-6">Loading SubCategory...</div>;

  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link to="/subcategories" className="p-2 text-charcoal/60 hover:text-charcoal bg-white rounded-lg shadow-sm border border-charcoal/5">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-charcoal/5 p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2 md:col-span-2">
            <label className="block text-sm font-medium text-charcoal">Parent Categories</label>
            <div className="flex flex-wrap gap-3 p-4 bg-ivory/30 rounded-lg border border-charcoal/10">
              {categories.length > 0 ? categories.map(category => (
                <label key={category._id} className="flex items-center gap-2 cursor-pointer bg-white px-3 py-2 rounded shadow-sm border border-charcoal/5 hover:border-brand transition-colors">
                  <input
                    type="checkbox"
                ...
                <div className="text-sm text-charcoal/50">No categories available.</div>
              )}
            </div>
          </div>
