...
export default function CategoryForm() {
  ...
  const fetchCategory = async () => {
    try {
      const { data } = await api.get(`/admin/catalog/categories/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await api.patch(`/admin/catalog/categories/${id}`, formData);
      } else {
  ...
  if (loading) return <div className="animate-pulse p-6">Loading Category...</div>;

  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link to="/categories" className="p-2 text-charcoal/60 hover:text-charcoal bg-white rounded-lg shadow-sm border border-charcoal/5">
