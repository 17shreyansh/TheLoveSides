...
export default function CollectionForm() {
  ...
  const fetchCollection = async () => {
    try {
      const { data } = await api.get(`/admin/catalog/collections/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await api.patch(`/admin/catalog/collections/${id}`, formData);
      } else {
        await api.post('/admin/catalog/collections', formData);
      }
  ...
  if (loading) return <div className="animate-pulse p-6">Loading collection...</div>;

  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link to="/collections" className="p-2 text-charcoal/60 hover:text-charcoal bg-white rounded-lg shadow-sm border border-charcoal/5">
